...
function fail(status, message, code) {
  return { status: status, body: { ok: false, message: message, code: code } };
}
...
async function createRequest(body) {
  ...
  if (!db.configured()) return fail(503, 'Supabase is not configured', 'NOT_CONFIGURED');

  /* 1. Hold the slot. The event type requires confirmation, so this
        lands as PENDING and the time disappears from the calendar. */
...
async function admin(action, body, cookieHeader) {
  ...
  if (!auth.configured()) {
    return fail(503, 'Admin access is not configured', 'NOT_CONFIGURED');
  }
  ...
  if (action === 'update') {
    ...
    if (b.status !== undefined) {
        ...
        if (current.row.cal_booking_uid) {
          ...
          if (!conf.ok) {
            return fail(conf.status || 502,
              'Marked nothing: Cal.com would not confirm the booking — ' + conf.message,
              'CONFIRM_FAILED');
          }
        }
      }
      ...
      if (next === 'cancelled' && current.row.status !== 'cancelled' && current.row.cal_booking_uid) {
        await cal.cancelBooking(current.row.cal_booking_uid, str(b.note, 200) || 'Request cancelled');
      }
    }
...
async function health() {
  var out = {
    ...
    admin:    { configured: auth.configured() },
    supabase: { configured: db.configured(), reachable: false, table: false }
  };
  ...
  var ready = out.cal.configured && out.admin.configured && out.supabase.table;
