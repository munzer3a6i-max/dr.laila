...
async function createRequest(body) {
  ...
  var saved = await db.createRequest({
    ...
    /* If Cal.com accepted the booking but we could not find a uid in the
       reply, the request can never be confirmed or cancelled from the
       dashboard. Mark it so that shows up on the card straight away rather
       than as a mystery when the admin marks it paid. */
    cal_status: booking.status || (booking.uid ? null : 'unlinked')
  });
...
async function admin(action, body, cookieHeader) {
  ...
  if (action === 'update') {
    ...
    var patch = {};
    /* What actually happened to the calendar, so the dashboard can say so
       instead of reporting a bare success for a status it only wrote to a
       database row. */
    var calendar = 'untouched';
    ...
    if (b.status !== undefined) {
      ...
      if (next === 'paid' && current.row.status !== 'paid') {
        ...
        if (!current.row.cal_booking_uid) {
          /* No Cal.com booking was ever linked to this request, so there is
             nothing to confirm and the time is not held in the calendar.
             The payment is still a fact and gets recorded — but silently
             succeeding here is what made this look like it worked. */
          calendar = 'none';
        } else {
      ...
      if (next === 'cancelled' && current.row.status !== 'cancelled' && current.row.cal_booking_uid) {
        ...
        if (cancelled.ok) { patch.cal_status = 'cancelled'; calendar = 'cancelled'; }
        else calendar = 'cancel-failed';
      }
    }

    var updated = await db.updateRequest(id, patch);
    if (!updated.ok) return fail(updated.status || 500, updated.message);
    return { status: 200, body: { ok: true, row: updated.row, calendar: calendar } };
  }
...
async function health() {
  ...
  if (out.supabase.table) {
    /* How many requests are holding time right now. If this is 0 while the
       dashboard shows live requests, the slot filter is reading a different
       database than the dashboard is writing to. */
    var live = await db.listRequests({ limit: 200 });
    if (live.ok) {
      out.supabase.liveHolds = live.rows.filter(function (r) { return r.status !== 'cancelled'; }).length;
      out.supabase.unlinked = live.rows.filter(function (r) {
        return r.status !== 'cancelled' && !r.cal_booking_uid;
      }).length;
    }
  }

  /* Whether CAL_API_KEY is set says nothing about whether it works, and a
     key that does not work fails at exactly one moment — when the admin
     marks a request paid and the confirm call is made. Find out here. */
  if (out.cal.configured) {
    var probe = await cal.getBooking('health-probe-nonexistent');
    /* 404 is the right answer: authenticated, and that booking is not real.
       401/403 means the key itself is being refused. */
    out.cal.keyWorks = probe.ok || probe.status === 404;
    if (!out.cal.keyWorks) out.cal.problem = probe.message;
  }
