...
var HASH_SHAPE = /^scrypt\$[0-9a-f]{32}\$[0-9a-f]{128}$/;

function configured() {
  return Boolean(process.env.ADMIN_PASSWORD_HASH) && Boolean(sessionKey());
}

/** Which required variables are absent — names only, never values. */
function missing() {
  var out = [];
  if (!process.env.ADMIN_PASSWORD_HASH) out.push('ADMIN_PASSWORD_HASH');
  if (!sessionKey()) out.push('SESSION_SECRET');
  return out;
}

/** Set but the wrong shape — usually a shell ate the $ signs in the hash. */
function hashMalformed() {
  var h = process.env.ADMIN_PASSWORD_HASH;
  return Boolean(h) && !HASH_SHAPE.test(h);
}
 ...
 *
 * SESSION_SECRET is preferred. If it is absent we derive one from
 * ADMIN_PASSWORD_HASH instead, which is itself a high-entropy
 * server-only value — so the dashboard works with one fewer variable
 * to get wrong. A derived key means changing the password signs any
 * open session out, which is the behaviour you would want anyway.
 */
function sessionKey() {
  var explicit = process.env.SESSION_SECRET;
  if (explicit && explicit.length >= 32) return explicit;

  var hash = process.env.ADMIN_PASSWORD_HASH;
  if (hash) {
    return crypto.createHmac('sha256', hash)
      .update('dr-dalal:session-key:v1').digest('hex');
  }
  return '';
}

/** 'explicit' | 'derived' | 'none' — reported by /api/health. */
function sessionKeySource() {
  var explicit = process.env.SESSION_SECRET;
  if (explicit && explicit.length >= 32) return 'explicit';
  if (process.env.ADMIN_PASSWORD_HASH) return 'derived';
  return 'none';
}
