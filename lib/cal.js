...
async function confirmBooking(uid) {
  ...
  if (!process.env.CAL_API_KEY) {
    return { ok: false, status: 503, message: 'CAL_API_KEY is required to confirm a booking' };
  }
...
async function cancelBooking(uid, reason) {
  if (!uid) return { ok: false, status: 400, message: 'booking uid is required' };
  if (!process.env.CAL_API_KEY) {
    return { ok: false, status: 503, message: 'CAL_API_KEY is required to cancel a booking' };
  }
