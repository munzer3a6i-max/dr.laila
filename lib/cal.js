...
async function getSlots(opts) {
  var eventTypeId = opts.eventTypeId;
  var start = opts.start;                 /* YYYY-MM-DD */
  var end = opts.end;                     /* YYYY-MM-DD */

  if (!eventTypeId) return { ok: false, status: 400, message: 'eventTypeId is required' };
...
async function createBooking(opts) {
  if (!opts.eventTypeId) return { ok: false, status: 400, message: 'eventTypeId is required' };
