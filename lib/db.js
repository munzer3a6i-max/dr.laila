...
function missing() {
  var out = [];
  if (!process.env.SUPABASE_URL) out.push('SUPABASE_URL');
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) out.push('SUPABASE_SERVICE_ROLE_KEY');
  return out;
}
...
async function createRequest(input) {
  ...
  if (input.cal_status) row.cal_status = input.cal_status;
...
/**
 * Requests that are still alive (anything but cancelled) and could overlap
 * a window, as {start, end} millisecond ranges.
 *
 * Cal.com is not a reliable lock here: a booking on an event type that
 * requires confirmation sits unconfirmed, and an unconfirmed booking does
 * not take its time off the calendar — which is exactly the state every
 * request is in until the admin marks it paid. So the site keeps its own
 * hold in this table, and that is what decides whether a slot is free.
 *
 * The overlap test needs each row's length, and PostgREST cannot filter on
 * starts_at + duration without a generated column, so the query casts a
 * wider net by start time and the arithmetic happens here. LOOKBACK_MS is
 * how far back a session could have started and still be running.
 */
var LOOKBACK_MS = 8 * 60 * 60 * 1000;
var DEFAULT_MINUTES = 50;

async function heldRanges(fromISO, toISO) {
  var from = new Date(fromISO);
  var to = new Date(toISO);
  if (isNaN(from) || isNaN(to)) return { ok: false, status: 400, message: 'Invalid range' };

  var q = 'requests?select=id,starts_at,duration_minutes,status'
        + '&status=neq.cancelled'
        + '&starts_at=gte.' + encodeURIComponent(new Date(from.getTime() - LOOKBACK_MS).toISOString())
        + '&starts_at=lte.' + encodeURIComponent(to.toISOString())
        + '&limit=1000';

  var out = await rest(q);
  if (!out.ok) return out;

  var ranges = (out.data || []).map(function (row) {
    var start = new Date(row.starts_at).getTime();
    var mins = Number(row.duration_minutes) || DEFAULT_MINUTES;
    return { id: row.id, start: start, end: start + mins * 60000 };
  }).filter(function (r) { return !isNaN(r.start); });

  return { ok: true, ranges: ranges };
}

/** Does [startISO, startISO + minutes) run into anything already held? */
async function findClash(startISO, minutes) {
  var start = new Date(startISO).getTime();
  if (isNaN(start)) return { ok: false, status: 400, message: 'Invalid start time' };
  var end = start + (Number(minutes) || DEFAULT_MINUTES) * 60000;

  var held = await heldRanges(new Date(start).toISOString(), new Date(end).toISOString());
  if (!held.ok) return held;

  var hit = held.ranges.find(function (r) { return r.start < end && r.end > start; });
  return { ok: true, clash: hit || null };
}
...
var UPDATABLE = ['status', 'admin_note', 'receipt_path', 'paid_at', 'cal_booking_uid', 'cal_status'];
